import pickle
import numpy as np
import os
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import LabelEncoder
import tensorflow as tf
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import LSTM, Dense, Dropout, Conv1D, MaxPooling1D, BatchNormalization, Flatten, TimeDistributed
from tensorflow.keras.utils import to_categorical
from tensorflow.keras.callbacks import EarlyStopping, ModelCheckpoint

# Configuration
PICKLE_PATH = "ai_training/wlasl_data.pickle"
MODEL_SAVE_PATH = "backend/trained_models/sign_model.h5"
LABELS_SAVE_PATH = "backend/trained_models/labels.txt"
SEQUENCE_LENGTH = 30
FEATURES_PER_FRAME = 63 # 21 landmarks * 3 (x,y,z)

def pad_or_truncate(sequence, length):
    if len(sequence) >= length:
        return sequence[:length]
    else:
        # Zero padding
        padding = [[0] * FEATURES_PER_FRAME for _ in range(length - len(sequence))]
        return sequence + padding

def train():
    print(f"Loading data from {PICKLE_PATH}...")
    if not os.path.exists(PICKLE_PATH):
        print("Data file not found!")
        return

    with open(PICKLE_PATH, "rb") as f:
        data_dict = pickle.load(f)
    
    raw_data = data_dict['data']
    raw_labels = data_dict['labels']
    
    print(f"Processing {len(raw_data)} sequences...")
    
    X = []
    for seq in raw_data:
        X.append(pad_or_truncate(seq, SEQUENCE_LENGTH))
    
    X = np.array(X)
    
    # Encode labels
    le = LabelEncoder()
    y_encoded = le.fit_transform(raw_labels)
    num_classes = len(le.classes_)
    
    # Save labels mapping
    with open(LABELS_SAVE_PATH, "w") as f:
        for label in le.classes_:
            f.write(label + "\n")
            
    y = to_categorical(y_encoded, num_classes=num_classes)
    
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, shuffle=True)
    
    print(f"Training on {X_train.shape} samples, validating on {X_test.shape}")
    
    # Model definition
    model = Sequential([
        # We use TimeDistributed Conv1D if we want to extract features per frame
        # but since features are already coordinates, 1D convolution over the features themselves might find spatial relationships
        Conv1D(64, kernel_size=3, padding='same', activation='relu', input_shape=(SEQUENCE_LENGTH, FEATURES_PER_FRAME)),
        BatchNormalization(),
        MaxPooling1D(pool_size=1),
        
        Conv1D(128, kernel_size=3, padding='same', activation='relu'),
        BatchNormalization(),
        MaxPooling1D(pool_size=1),
        
        LSTM(128, return_sequences=True),
        LSTM(64),
        
        Dense(256, activation='relu'),
        Dropout(0.4),
        Dense(num_classes, activation='softmax')
    ])
    
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    
    print(model.summary())
    
    # Callbacks
    early_stop = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
    checkpoint = ModelCheckpoint(MODEL_SAVE_PATH, monitor='val_accuracy', save_best_only=True, mode='max')
    
    # Training (Small epochs for demonstration, ideally more)
    model.fit(
        X_train, y_train, 
        epochs=30, 
        batch_size=32, 
        validation_data=(X_test, y_test),
        callbacks=[early_stop, checkpoint]
    )
    
    print(f"Training complete. Model saved to {MODEL_SAVE_PATH}")
    print(f"Labels saved to {LABELS_SAVE_PATH}")

if __name__ == "__main__":
    train()
